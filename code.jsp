...
<%
        ...
        java.io.PrintWriter log = new java.io.PrintWriter(new java.io.FileWriter(gen.getDataLogFileQuery(), true), true);
          ...
          String title="Code "+tp;       
          String[] cond=new String[0];   
            String[] judul=new String[]{"SEQ","CODE","DESCRIPTION"};  //
            String COL= "'Id','Code','Description'";
        java.util.Vector data=sgen.getDataQuery(conn,"CODE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),gen.gettable(request.getParameter("Y11"))});
        if(gen.gettable(request.getParameter("Y11")).equalsIgnoreCase("SHP")||gen.gettable(request.getParameter("Y11")).equalsIgnoreCase("CSN")||gen.gettable(request.getParameter("Y11")).equalsIgnoreCase("PORT")){
            data=sgen.getDataQuery(conn,"CODENOER",new String[]{gen.gettable(request.getParameter("Y11"))});
        }
        int hitx=1;
        for(int s=0;s<data.size();s+=3){
        ...
        ses.setAttribute("CODE",data);
            if(gen.gettable(request.getParameter("tpx")).equalsIgnoreCase("Download")){
                  ...
                  java.util.Vector judulx=gen.getElement(',',"SEQ,CODE,DESCRIPTION,");                  
...
<![endif]-->
		...
		<script type="text/javascript">
            ...
            function down(){
                location.href="code.jsp?tp=<%=tp%>&tb=<%=gen.gettable(request.getParameter("Y11"))%>&Y11=<%=gen.gettable(request.getParameter("Y11"))%>&tpx=Download";
            }
