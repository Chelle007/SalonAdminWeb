        ...
        if(Gen.gettable(request.getParameter("act")).equalsIgnoreCase("Save")){
            ...
            if(Gen.gettable(request.getParameter("add")).equalsIgnoreCase("true")){//Umum
                ...
                for (int i = 2; i < addArray.length; i++) {
                    addArray[i - 1] = gen.gettable(request.getParameter(judul[i]));
                }
...
*/
        ...
        java.util.Vector vk=sgen.getDataQuery(conn,tp,new String[0]);
        String add="false";
        if(vk.size()==0){
        ...
        java.util.Hashtable<String, java.util.Vector> detailDataMap = new java.util.Hashtable<>();
        if(!add.equalsIgnoreCase("true")) {
            for (int i = 0; i < detailTp.length; i++) {
                detailDataMap.put(detailTp[i], sgen.getDataQuery(conn, detailTp[i], new String[]{S1}));
            }
        }
        ...
        if(request.getParameter("act")!=null && !Gen.gettable(request.getParameter("act")).equalsIgnoreCase("Save")){            //  [SALES_ID],[SALES_DATE] ,[MEMBER_ID],[AGENT_ID],[CARD_ID],[CARD_PACKAGE_ID],[TOTAL_PRICE],[TOTAL_DISCOUNT_AMOUNT], [REMARK],member_name ,agent_name,card_name,voucher_no  FROM 
            ...
            for (int i = 0; i < detailTp.length; i++) {
                int rowCount=gen.getInt(request.getParameter("CS"+detailTp[i]));
                ...
                for(int m=1;m<=rowCount;m++){// remark yuli act=DelSales artinya tekan tombol delete di salah satu item
                    ...
                    }else{//remark yuli kondisi selain delete item, simpan data2 dari parameter yang sudah di input user,supaya tidak hilang
                      tmp.addElement(request.getParameter("B"+(i+1)+"b"+m));
                      System.out.println("B"+(i+1)+"b"+m+" "+request.getParameter("B"+(i+1)+"b"+m));
                      tmp.addElement(request.getParameter("C"+(i+1)+"c"+m));
                      tmp.addElement(request.getParameter("D"+(i+1)+"d"+m));
                    }
                }
                detailDataMap.put(detailTp[i], tmp);
                System.out.println(detailTp[i] + " " + rowCount + " " + tmp);
            }
            ...
            if (gen.gettable(request.getParameter("act")).equalsIgnoreCase("AddRowS_TREATMENT_ITEMS")) {
                java.util.Vector detailData = detailDataMap.get("S_TREATMENT_ITEMS");
                for (int i = 0; i < 5; i++) {
                    detailData.addElement("");
                }
                detailDataMap.put("S_TREATMENT_ITEMS", detailData); // Update the vector in the map
            } else if (gen.gettable(request.getParameter("act")).equalsIgnoreCase("AddRowS_CARD_ALLOWED_ITEM")) {
                java.util.Vector detailData = detailDataMap.get("S_CARD_ALLOWED_ITEM");
                for (int i = 0; i < 2; i++) {
                    detailData.addElement("");
                }
                detailDataMap.put("S_CARD_ALLOWED_ITEM", detailData); // Update the vector in the map
            } else if (gen.gettable(request.getParameter("act")).equalsIgnoreCase("AddRowS_CARD_ALLOWED_TREATMENT")) {
                java.util.Vector detailData = detailDataMap.get("S_CARD_ALLOWED_TREATMENT");
                for (int i = 0; i < 3; i++) {
                    detailData.addElement("");
                }
                detailDataMap.put("S_CARD_ALLOWED_TREATMENT", detailData); // Update the vector in the map
            } else if (gen.gettable(request.getParameter("act")).equalsIgnoreCase("AddRowS_CARD_PACKAGE_DETAIL")) {
                java.util.Vector detailData = detailDataMap.get("S_CARD_PACKAGE_DETAIL");
                for (int i = 0; i < 3; i++) {
                    detailData.addElement("");
                }
                detailDataMap.put("S_CARD_PACKAGE_DETAIL", detailData); // Update the vector in the map
            }
        }
				...
				<div class="main-content-inner">
					...
					<div class="page-content">
                        ...
                        for (int h=0; h<detailTp.length; h++) {
                            ...
                            String detailPrefix = "";
                            // ubah sini
                            if ("S_TREATMENT_ITEMS".equals(detailTp[h])) {
                                judulDetail = new String[]{"Item_id", "Quantity", "Purchase_price_per_unit", "Sales_price_per_unit"};
                                maplistType = "ITEM";
                                detailPrefix = "TI";
                            } else if ("S_CARD_ALLOWED_ITEM".equals(detailTp[h])) {
                                judulDetail = new String[]{"Item_id"};
                                maplistType = "ITEM";
                                detailPrefix = "CAI";
                            } else if ("S_CARD_ALLOWED_TREATMENT".equals(detailTp[h])) {
                                judulDetail = new String[]{"Treatment_id", "Sales_price"};
                                maplistType = "TREATMENT";
                                detailPrefix = "CAT";
                            } else if ("S_CARD_PACKAGE_DETAIL".equals(detailTp[h])) {
                                judulDetail = new String[]{"Card_id", "Quantity"};
                                maplistType = "CARD";
                                detailPrefix = "CPD";
                            }

                            java.util.Vector detailData = detailDataMap.get(detailTp[h]);
                        ...
                        <div class="row">
                            <table id="item" class="table  table-bordered table-hover">
                				<thead>
                					<tr>
                                        <th colspan=<%=judulDetail.length+1%>><%=detailTitle[h]%> Information<input type="hidden" name="CS<%=detailTp[h]%>" value="<%=(detailData.size()/(judulDetail.length+1))%>"></th>
                    				</tr>
                					<tr>
                                    ...
                                    %>
                						<tr>
                							<td nowrap>
                               		          <input type="text" name="B<%=(h+1)%>b<%=hit%>" size="5"  value="<%=Gen.gettable(detailData.get(i)).trim()%>" disabled>
                          		              <input type="text" name="C<%=(h+1)%>c<%=hit%>" size="20"  value="<%=Gen.gettable(detailData.get(i+1)).trim()%>" disabled>
                                              <%System.out.println("B"+(h+1)+"b"+hit+" "+Gen.gettable(detailData.get(i)).trim());%>
                                              <%if(view.length()==0){%>
                   				                  <input type="button" value="..." name="act1" style="font-weight: bold" tabindex="5" onClick="setLink2('<%=maplistType%>','B<%=(h+1)%>b<%=hit%>','C<%=(h+1)%>c<%=hit%>','','','E<%=(h+1)%>e<%=hit%>')">
                                              <%}%>
                                            </td>
                                            ...
                                            %>
                                            <td>
                                                <input type="text" style="text-align:right;" id="D<%=(h+1)%>d<%=hit%>" name="D<%=(h+1)%>d<%=hit%>" maxlength="3" size="10" value="<%=Gen.getNumberFormat(detailData.get(j),0)%>" onchange="cc(D<%=(h+1)%>d<%=hit%>,E<%=(h+1)%>e<%=hit%>,F<%=(h+1)%>f<%=hit%>)"  <%=view%>/>
                                            </td nowrap>
                                            <%}%>

                                            <td nowrap><button class="btn btn-xs btn-danger"  onclick="ondel('<%=detailTp[h]%>','<%=hit%>')">
  												<i class="ace-icon fa fa-trash-o bigger-120"></i></button>
                                            </td>
                   						</tr>
                                    <%hit++;
                                    }%>
                                    <tr><td colspan=<%=judulDetail.length%>></td><td><a href="javascript:subm('AddRow<%=detailTp[h]%>')"><img width=20 height=20 src=image/plus.gif></a></td></tr>
                				</tbody>
                			</table>
                        </div>
                		<%}%>
...
//                                                            T13=16 [Vendor],T14=17 [BillTo],T15=18 [Customer],T16=[MYOBNO],T17=[MYOBAMT],T18=[MYOBSTATUS]
            ...
            function opendis(){
                     ...
                     */
                     <%
                     for(int i=1; i<=detailTp.length; i++){
                        int rowCount=gen.getInt(request.getParameter("CS"+detailTp[i]));
                        for (int j=1; j<=rowCount; j++) {
                        %>
                            document.BGA.B<%=i%>b<%=j%>.disabled=false;
                            document.BGA.C<%=i%>c<%=j%>.disabled=false;
                            document.BGA.D<%=i%>d<%=j%>.disabled=false;
                        <%}
                     }%>
            }
            function  refresh(){                
