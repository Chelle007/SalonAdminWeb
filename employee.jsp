...
<%
        ...
        java.io.PrintWriter log = new java.io.PrintWriter(new java.io.FileWriter(gen.getDataLogFileQuery(), true), true);
          ...
          String[] cond=new String[0];
            String[] judul=new String[]{"SEQ","CODE","DESCRIPTION"};  //ubah sini judul tiap kolom
            String COL= "'Id','Code','Description'";//ubah sini untuk judul screen
        java.util.Vector data=sgen.getDataQuery(conn,tp,new String[0]);
        int hitx=1;
        for(int s=0;s<data.size();s+=3){//ubah sini kalo 3 jumlah kolom tujuan ini untuk set kolom pertama (id sequensial)
            data.setElementAt(hitx+"",s);
            hitx++;
        }
        ses.setAttribute(tp,data);
            if(gen.gettable(request.getParameter("tpx")).equalsIgnoreCase("Download")){
                  com.ysoft.BUILDEXCEL bd=new com.ysoft.BUILDEXCEL();
                  java.util.Vector judulx=gen.getElement(',',"SEQ,CODE,DESCRIPTION,");//ubah sini untuk judul2 kolom2 di excel
...
<![endif]-->
		...
		<script type="text/javascript">
            ...
            function down(){ // TODO: change employee to jsp file
                location.href="employee.jsp?tp=<%=tp%>&tb=<%=gen.gettable(request.getParameter("Y11"))%>&Y11=<%=gen.gettable(request.getParameter("Y11"))%>&tpx=Download";
            }

        	function pop (){
        		window.open("download/<%=gen.gettable(request.getParameter("Y11"))%>.xls","","toolbar=yes,menubar=yes,resizable=1,scrollbars=1");
        	}
            ...
            <%
            int hit=1;
            for(int m=0;m<data.size();m+=judul.length){//ubah di sini untuk coding grid data2nya
                  if(m==data.size()-judul.length){                   //'Id Anggota','Nama','Jenis Kelamin', 'Jenis Anggota', 'Donatur','NO HP','NO WA'
                      out.println("{id:\""+gen.gettable(data.get(m)).trim()+"\",code:\""+gen.gettable(data.get(m+1)).trim()+"\",description:\""+gen.gettable(data.get(m+2)).trim()+"\"}");
                  }else{
                      out.println("{id:\""+gen.gettable(data.get(m)).trim()+"\",code:\""+gen.gettable(data.get(m+1)).trim()+"\",description:\""+gen.gettable(data.get(m+2)).trim()+"\"},");
                  }
                hit++;
...
//ubah sini komponen untuk setiap kolom 
					data: grid_data,
					datatype: "local",
					height: 250,
					colNames:['Edit',<%=COL%>],
