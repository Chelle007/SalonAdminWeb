...
<%
    ...
    switch (tp) { // ubah sini
        ...
        case "S_TREATMENT_ITEMS":
            judul = new String[]{"ID", "TI_ID", "Treatment_id", "Item_id", "Quantity", "Purchase_price_per_unit", "Sales_price_per_unit"};
            break;
        case "S_CARD_ALLOWED_ITEM":
            judul = new String[]{"ID", "CAI_ID", "Card_id", "Item_id"};
            break;
        case "S_CARD_ALLOWED_TREATMENT":
            judul = new String[]{"ID", "CAT_ID", "Card_id", "Treatment_id"};
            break;
        case "S_CARD_PACKAGE_DETAIL":
            judul = new String[]{"ID", "CPD_ID", "Card_package_id", "Card_id", "Quantity"};
            break;
            */
    }
										...
										<table id="simple-table" class="table  table-bordered table-hover">
											<thead>
												<tr>													
                                                        ...
                                                        %>         
														<th><%=judul[s]%>&nbsp;<%if(s==0){%><a href="master2detail.jsp?add=true&tp=<%=tp%>&title=<%=title%>"><img width=20 height=20 src=image/plus.gif></a><%}%></th>
                                                        <%}%>
                                                    <th>DELETE</th>
    											</tr>
											</thead>
											...
											<tbody>
												...
												<tr>
                                                       ...
                                                       %>
                                                            ...
                                                            <%if(ss==0){%>
                                                            <a href="master2detail.jsp?tp=<%=tp%>&title=<%=title%>&canedit=true&S1=<%=gen.gettable(data.get(s+ss)).trim()%>"><%=gen.gettable(data.get(s+ss)).trim()%></a>
                                                            <%}else{
                                                                if(ss<6){
		...
		window.open(url,"", "height=600,width=900,toolbar=no,scrollbars=yes,menubar=no");
            ...
            function ondel(Parm){
                if(confirm("Delete Selected Record?","Delete")){
                    location.href="master2.jsp?act=DEL&Y10="+document.BGA.Y10.value+"&Y11="+document.BGA.Y11.value+"&Y12="+document.BGA.Y12.value+"&Y13="+document.BGA.Y13.value+"&Y15="+document.BGA.Y15.value+"&Y14="+document.BGA.Y14.value+"&tp=<%=request.getParameter("tp")%>&title=<%=title%>&S1="+Parm;
                }
            }
            function upload(){
