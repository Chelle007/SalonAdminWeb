      ...
      //      System.out.println(pr+"="+Gen.gettable(request.getParameter(pr)));
        ...
        if(Gen.gettable(request.getParameter("act")).equalsIgnoreCase("Save")){
            ...
            //[ErCode],[JobNo],[Seq],[BillTo],[AcctNo],[DK],[Curr],[Amt],[CheckFlag],[IDRAmt] ,[Perc],[FinalAmt],[MYOB],[percflag],[rate]
                ...
                int hsales=1,hcost=1;
                for(int ms=0;ms<oldcost.size();ms+=15){//
                    if(gen.getInt(oldcost.get(ms+7))>hcost){//ada accrual
                        hcost=gen.getInt(oldcost.get(ms+7));
                    }
                }
                hcost++;
                
                  for(int ms=0;ms<oldsales.size();ms+=14){//
                    if(gen.getInt(oldsales.get(ms+7))>hsales){//ada accrual
                        hsales=gen.getInt(oldsales.get(ms+7));
                    }
                  }
                hsales++;
                
                if(msg.length()==0) msg=sgen.update(conn,"JOBCOSTACCTDELETE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),y1});
                if(msg.length()==0) msg=sgen.update(conn,"JOBSALESACCTDELETE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),y1});
                ...
                for(int m=1;m<=hx;m++){
                    if(gen.gettable(request.getParameter("A"+m)).length()>0 || gen.gettable(request.getParameter("B"+m)).length()>0){
                        ...
                        String[] cc=new String[]{gen.gettable(ses.getAttribute("TxtErcode")),y1,hsales+"",gen.gettable(request.getParameter("A"+m)),
                        ...
                        if(cek.equalsIgnoreCase("1")) cc[16]=gen.gettable(request.getParameter("AT2"));
                        if(msg.length()==0) msg=sgen.update(conn,"JOBSALESACCTADD",cc);
                        if(msg.length()==0) msg=sgen.update(conn,"LOGMENUPARMADD",new String[]{gen.gettable(ses.getAttribute("User")),"JOBSALESACCTADD",y1+"|"+hsales+"|"+gen.gettable(request.getParameter("A"+m))+"|"+gen.gettable(request.getParameter("B"+m))+"|"});
                        hsales++;
                    }
                }
                if(msg.length()==0) {
                  ...
                  for(int m=1;m<=hx;m++){//[ErCode] ,[JobNo] ,[Seq] ,[Vendor],[AcctNo],[Invoice] ,[CurrCost],[AmtCost],[CheckFlag],[IDRAmtCost],[PercCost],[FinalCost],[MYOB],[percflag],[rate]
                      if(gen.gettable(request.getParameter("K"+m)).length()>0 || gen.gettable(request.getParameter("L"+m)).length()>0){
                          ...
                          String[] cc=new String[]{gen.gettable(ses.getAttribute("TxtErcode")),y1,hcost+"",
                          ...
                          if(cek.equalsIgnoreCase("1")) cc[16]=gen.gettable(request.getParameter("AT2"));
                          if(msg.length()==0) msg=sgen.update(conn,"JOBCOSTACCTADD",cc);
                          if(msg.length()==0) msg=sgen.update(conn,"LOGMENUPARMADD",new String[]{gen.gettable(ses.getAttribute("User")),"JOBCOSTACCTADD",y1+"|"+hcost+"|"+gen.gettable(request.getParameter("K"+m))+"|"+gen.gettable(request.getParameter("L"+m))+"|"});
                          hcost++;
                      }
                  }
                }
                if(msg.length()==0){
                      String[] cd=new String[]{gen.gettable(ses.getAttribute("TxtErcode")),y1};
        ...
        if(!add.equalsIgnoreCase("true")) sales=sgen.getDataQuery(conn,"JOBACCTSALES",cond);
        java.util.Vector cost=new java.util.Vector();
        if(!add.equalsIgnoreCase("true")) cost=sgen.getDataQuery(conn,"JOBACCTCOST",cond);
    ...
    <%}%>
			<div class="main-content">
				<div class="main-content-inner">
			         <form class="form-horizontal" name="BGA" method="POST" action="jobcompleted.jsp?tp=<%=Gen.gettable(request.getParameter("tp"))%>" >
					<div class="page-content">
						...
						<div class="row">
                            <div id="user-profile-3" >
                              <table id="simple-table" class="table  table-bordered table-hover">
  								<thead>
  									<tr><td colspan=4><b>Job Header Information</b><td></tr>
  								</thead>                
  								<tbody>
                                      <tr>
                                          <td>Date:</td><td nowrap><input name="T2"  type="hidden" value="<%=Gen.gettable(vk.get(3)).trim()%>"/><%=Gen.gettable(vk.get(3)).trim()%></td>
                                      ...
                                      <tr>
                                          <td>Job No:</td><td nowrap><input type="hidden" id="T1" name="T1"  value="<%=y1%>" /><b><%=y1%></B></td>
                                      ...
                                      <tr>
                                          <td>Accrual Date :</td><td nowrap><input name="AT2"   class="input-medium date-picker"  id="AT2" type="text" data-date-format="dd-mm-yyyy" value="<%=accrual%>" placeholder="dd-mm-yyyy" /><i class="ace-icon fa fa-calendar"></i></td>
                                      ...
                                      <tr>
                                          <td >Customer:</td><td nowrap><input type="hidden" name="T5" value="<%=Gen.gettable(vk.get(6))%>"><input type="text" name="ST5" size="45" maxlength="60" tabindex="10"  value="<%=Gen.gettable(vk.get(13)).trim()%>" disabled>
                                          </td>
                        ...
                        <div class="row">
                            <table id="Sales" class="table  table-bordered table-hover">
                				<thead>
                					<tr>
                                        <th colspan=9>Sales Information<input type="hidden" name="CS" value="<%=(sales.size()/12)%>"></th>
                    				</tr>
                					<tr>
                                        <th>BILL TO</th>
                                        <th>ACCOUNT</th>
                                    ...
                                    for(int i=0;i<sales.size();i+=12){
                                        if(Gen.gettable(sales.get(i+4)).trim().equalsIgnoreCase("1")){%>
                						<tr>
                							<td nowrap><%=Gen.gettable(sales.get(i)).trim()%>(<%=Gen.gettable(sales.get(i+9)).trim()%>)</td>
                                            <td nowrap><%=Gen.gettable(sales.get(i+1)).trim()%></td>
                                            <td nowrap><%=Gen.gettable(sales.get(i+2)).trim()%></td>
                                            <td nowrap align=right><%=Gen.getNumberFormat(sales.get(i+10),0)%></td>
                                            <td nowrap align=right><%=Gen.getNumberFormat(sales.get(i+3))%></td>
                                            <td nowrap><%if(Gen.gettable(sales.get(i+4)).trim().equalsIgnoreCase("1")) out.print("Yes");%></td>
                                            <td nowrap align=right><%=Gen.getNumberFormat(sales.get(i+5),0)%></td>
                                            <td nowrap><%=Gen.gettable(sales.get(i+11)).trim()%></td>
                                            <td nowrap><%=Gen.gettable(sales.get(i+8)).trim()%></td>
                                        </tr>
<%                                      }else{%>
                						<tr>
                							<td nowrap>
                                    ...
                                    int hitsales=hit-1;
                                    %>
                                    <tr><td colspan=8 align=right></td><td><a href="javascript:subm('AddSales')"><img width=20 height=20 src=image/plus.gif></a></td></tr>
                				</tbody>
                			</table>
                        </div>
                        <div class="row">
                            <table id="Sales" class="table  table-bordered table-hover">
                				<thead>
                					<tr>
                                        <th colspan=10 >Cost Information<input type="hidden" name="CC" value="<%=(cost.size()/13)%>"></th>
                    				</tr>
                					<tr>
                                        <th>VENDOR</th>
                                        <th>ACCOUNT</th>
                                    ...
                                    for(int i=0;i<cost.size();i+=13){
                                        if(Gen.gettable(cost.get(i+5)).trim().equalsIgnoreCase("1")){%>
                						<tr>
                							<td nowrap><%=Gen.gettable(cost.get(i)).trim()%>(<%=Gen.gettable(cost.get(i+10)).trim()%>)</td>
                                            <td nowrap><%=Gen.gettable(cost.get(i+1)).trim()%></td>
                                            <td nowrap><%=Gen.gettable(cost.get(i+2)).trim()%></td>
                                            <td nowrap><%=Gen.gettable(cost.get(i+3)).trim()%></td>
                                            <td nowrap align=right><%=Gen.getNumberFormat(cost.get(i+11),0)%></td>
                                            <td nowrap align=right><%=Gen.getNumberFormat(cost.get(i+4))%></td>
                                            <td nowrap><%if(Gen.gettable(cost.get(i+5)).trim().equalsIgnoreCase("1")) out.print("Yes");%></td>
                                            <td nowrap align=right><%=Gen.getNumberFormat(cost.get(i+6),0)%></td>
                                            <td nowrap><%=Gen.gettable(cost.get(i+12)).trim()%></td>
                                            <td nowrap><%=Gen.gettable(cost.get(i+9)).trim()%></td>
                                        </tr>
<%                                      }else{%>
                						<tr>
                							<td nowrap>
                                    ...
                                    <tr><td colspan=9 align=right></td><td><a href="javascript:subm('AddCost')"><img width=20 height=20 src=image/plus.gif></a></td></tr>
                				</tbody>
                			</table>
						</div><!-- /.row -->
                      <%if(view.length()==0){%>
...
<![endif]-->
    ...
    function onreturn(){
        location.href="jobacct.jsp?tp=JOBACCT";
    }
...
//                                                            T13=16 [Vendor],T14=17 [BillTo],T15=18 [Customer],T16=[MYOBNO],T17=[MYOBAMT],T18=[MYOBSTATUS]
            ...
            function  setsave(Parm){
                if(Parm=="1"){
                  if(document.BGA.AT2.value==""){
                      alert("Accrual Date must be filled!");
                  }else{
                     <%
                     
                     for(int i=1;i<=hitsales;i++){%>
                        document.BGA.B<%=i%>.disabled=false;
                        document.BGA.Z<%=i%>.disabled=false;
                     <%
                     }%>
                     <%
                     
                     for(int i=1;i<hit;i++){%>
                        document.BGA.L<%=i%>.disabled=false;
                        document.BGA.X<%=i%>.disabled=false;
                     <%
                     }%>
            ...
            function counttot(){
                     ...
                     for(int i=0;i<sales.size();i+=12){
                        if(Gen.gettable(sales.get(i+4)).trim().equalsIgnoreCase("1")){
                            out.print("t1=t1+"+gen.getformatdouble(sales.get(i+5))+";");
                        }else{
                     %>
                     ...
                     for(int i=0;i<cost.size();i+=13){
                        if(Gen.gettable(cost.get(i+5)).trim().equalsIgnoreCase("1")){
                            out.print("t2=t2+"+gen.getformatdouble(cost.get(i+6))+";");
                        }else{
                     %>
            ...
            function countdet(){
                     ...
                     for(int i=1;i<=hitsales;i++){%>
                        if(document.BGA.C<%=i%>.value!='IDR'){
                            document.BGA.F<%=i%>.value=NumberFormat(Digit(document.BGA.D<%=i%>.value)*Digit(document.BGA.J<%=i%>.value));
                        }
                     <%
                     }%>
                     var t2=0,pt2=0;
                     ...
                     for(int i=1;i<hit;i++){%>
                        if(document.BGA.N<%=i%>.value!='IDR'){
                            document.BGA.Q<%=i%>.value=NumberFormat(Digit(document.BGA.U<%=i%>.value)*Digit(document.BGA.O<%=i%>.value));
                        }
                     <%
                     }%>
                     counttot();
