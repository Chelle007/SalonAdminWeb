        ...
        java.io.PrintWriter log = new java.io.PrintWriter(new java.io.FileWriter(gen.getDataLogFileQuery(), true), true);
         ...
         String msg="";
            if(Gen.gettable(request.getParameter("act")).equalsIgnoreCase("Save")){
                ...
                msg=sgen.update(conn,"ARPAYMENTDELETE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),Gen.gettable(request.getParameter("S5")),Gen.gettable(request.getParameter("S6"))});
                //[ErCode] ,[BillTo] ,curr,[seq],[jobno],[paiddate],[paidamt],[bank],[payno],[refno]
                if(msg.length()==0) {
                    ...
                    for(int m=1;m<=hi;m++){
                        ...
                        if(paid.length()>0){
                            ...
                            if(msg.length()==0) msg=sgen.update(conn,"ARBANKPAYMENTDELETE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),Gen.gettable(request.getParameter("S6")),refno,payno});
/*                            if(GL.length()>0){
                            ...
                            if(msg.length()==0) msg=sgen.update(conn,"ARPAYMENTADD",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),Gen.gettable(request.getParameter("S5")),Gen.gettable(request.getParameter("S6")),paid,amt,bank,payno,refno});
                            ...
                            String[] xc=new String[]{Gen.gettable(request.getParameter("S9")),"0",amt,"1","SALES",payno,refno,Gen.gettable(request.getParameter("S6")),Gen.gettable(request.getParameter("S2")),payno,gen.gettable(ses.getAttribute("TxtErcode")),bank,paid,SEQ,gen.gettable(request.getParameter("S4")),"0",Gen.gettable(request.getParameter("S1"))};
                            String rate="1";
                            ...
                            if(msg.length()==0) msg=sgen.update(conn,"TRX_BANKUPDATE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),bank,paid,SEQ});
                            //[ErCode] ,[BillTo] ,curr,[seq],[jobno],[ErCode] ,[BillTo] ,curr,[seq],[jobno]
                            if(msg.length()==0) msg=sgen.update(conn,"ARPAYUPDATE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),Gen.gettable(request.getParameter("S5")),Gen.gettable(request.getParameter("S6")),gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),Gen.gettable(request.getParameter("S5")),Gen.gettable(request.getParameter("S6"))});
                            if(msg.length()==0) msg=sgen.update(conn,"LOGMENUPARMADD",new String[]{gen.gettable(ses.getAttribute("User")),"ARPAYMENTADD",gen.gettable(ses.getAttribute("TxtErcode"))+"|"+gen.gettable(request.getParameter("S1"))+"|"+Gen.gettable(request.getParameter("S4"))+"|"+Gen.gettable(request.getParameter("S5"))+"|"+Gen.gettable(request.getParameter("S6"))+"|"+paid+"|"+payno+"|"+refno+"|"});
                       ...
                       /*                     //[ErCode] ,[GLId],[TrxDate],curr,[Remarks]
                            ...
                            GLID=yr+mth+GLID;
                            xc=new String[]{gen.gettable(ses.getAttribute("TxtErcode")),GLID,paid,gen.gettable(request.getParameter("S4")),"Account Receivable"};
                            if(msg.length()==0) msg=sgen.update(conn,"GLADD",xc);
                            if(msg.length()==0) msg=sgen.update(conn,"ARPAYMENTUPDATE",new String[]{GLID,gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),Gen.gettable(request.getParameter("S5")),Gen.gettable(request.getParameter("S6")),paid,payno});
                            ...
                            if(acctb.size()>0){
                                ...
                                xc=new String[]{gen.gettable(ses.getAttribute("TxtErcode")),GLID,Gen.gettable(request.getParameter("S9")),"0",amt,rate,"0",idramt+"",Gen.gettable(request.getParameter("S6")),"Sales",""};
                                if(msg.length()==0) msg=sgen.update(conn,"GLDETAILADD",xc);
                                xc=new String[]{gen.gettable(ses.getAttribute("TxtErcode")),GLID,Gen.gettable(acctb.get(0)).trim(),amt,"0",rate,idramt+"","0",Gen.gettable(request.getParameter("S6")),Gen.gettable(acctb.get(1)).trim(),""};
                                if(msg.length()==0) msg=sgen.update(conn,"GLDETAILADD",xc);
                            }
                            
                            if(msg.length()==0) msg=sgen.update(conn,"LOGMENUPARMADD",new String[]{gen.gettable(ses.getAttribute("User")),"ARGLADD",gen.gettable(ses.getAttribute("TxtErcode"))+"|"+GLID+"|"});
*/
            ...
            /*    if(Gen.gettable(request.getParameter("P4")).length()>0){
                ...
                msg=sgen.update(conn,"ARPAYMENTDDDELETE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),Gen.gettable(request.getParameter("S5")),Gen.gettable(request.getParameter("S6")),Gen.gettable(request.getParameter("P1")),Gen.gettable(request.getParameter("P2")),Gen.gettable(request.getParameter("P3"))});
                if(msg.length()==0) msg=sgen.update(conn,"ARBANKPAYMENTDDDELETE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),Gen.gettable(request.getParameter("S6")),Gen.gettable(request.getParameter("P1")),Gen.gettable(request.getParameter("P2")),Gen.gettable(request.getParameter("P3"))});
                if(msg.length()==0) msg=sgen.update(conn,"ARPAYUPDATE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),Gen.gettable(request.getParameter("S5")),Gen.gettable(request.getParameter("S6")),gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),Gen.gettable(request.getParameter("S5")),Gen.gettable(request.getParameter("S6"))});
                if(msg.length()==0) msg=sgen.update(conn,"LOGMENUPARMADD",new String[]{gen.gettable(ses.getAttribute("User")),"ARPAYMENTDDDELETE",gen.gettable(ses.getAttribute("TxtErcode"))+"|"+gen.gettable(request.getParameter("S1"))+"|"+Gen.gettable(request.getParameter("S4"))+"|"+Gen.gettable(request.getParameter("S5"))+"|"+Gen.gettable(request.getParameter("S6"))+"|"+Gen.gettable(request.getParameter("P1"))+"|"+Gen.gettable(request.getParameter("P2"))+"|"+Gen.gettable(request.getParameter("P3"))+"|"});
                if(msg.length()>0){
                    conn.rollback();
                    msg="(Save Data Failed!!"+msg+")";
                }else{
         ...
         String[] judul=new String[]{"PAID DATE","PAID AMOUNT","BANK","PAY NO.","REF. NO.",""};
          ...
          String title="AR Payment Detail";       
...
%>
    ...
    <jsp:include page="menu.jsp" flush ="true"/>
			<div class="main-content">
				...
				<form class="form-horizontal" name="BGA" method="POST" action="ARPay.jsp?tp=<%=tp%>" >
								...
								<div>
									<div class="user-profile row">
                                        <div class="col-sm-offset-1 col-sm-10">
                                            ...
                                            <input type="hidden" name="canedit" value="<%=gen.gettable(request.getParameter("canedit"))%>" >
								...
								<!-- PAGE CONTENT BEGINS -->
										<table id="simple-table" class="table  table-bordered table-hover">
											...
											<tbody>
												...
												<tr>
                                                      <td>                                                       
                                                       <input class="input-medium date-picker" name="A<%=hic%>"  id="A<%=hic%>" type="text" data-date-format="dd-mm-yyyy" value="<%=gen.gettable(data.get(s))%>" placeholder="dd-mm-yyyy"  <%=view%>/><i class="ace-icon fa fa-calendar"></i></td>                                                            
                                                      <td><input type="text" name="B<%=hic%>" size="10" maxlength="10" value="<%=gen.getNumberFormat(data.get(s+1),2)%>" <%=view%>></td>
                                                      <TD><select name="C<%=hic%>"  <%=view%>>
                                                          <option></option>
                                                      ...
                                                      <td><input type="text" name="D<%=hic%>" size="15" maxlength="20" value="<%=gen.gettable(data.get(s+3)).trim()%>"  <%=view%>></td>
                                                      <td><input type="text" name="E<%=hic%>" size="15" maxlength="20" value="<%=gen.gettable(data.get(s+4)).trim()%>"  <%=view%>>
                                                      <input type="hidden" name="F<%=hic%>" value="<%=gen.gettable(data.get(s+5)).trim()%>"  <%=view%>></td>
                                                    <td nowrap>
...
<![endif]-->
     ...
     function onreturn(){
        location.href="ARDetail.jsp?tp=ARDETAIL&canedit=true&Y10=<%=y10%>&Y11=<%=y11%>&Y14=<%=Gen.gettable(request.getParameter("Y14"))%>&S1=<%=Gen.gettable(request.getParameter("S1"))%>&S2=<%=Gen.gettable(request.getParameter("S2"))%>&S3=<%=Gen.gettable(request.getParameter("S3"))%>&S4=<%=Gen.gettable(request.getParameter("S4"))%>";
    }
