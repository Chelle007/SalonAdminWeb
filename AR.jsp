...
<%
        ...
        java.io.PrintWriter log = new java.io.PrintWriter(new java.io.FileWriter(gen.getDataLogFileQuery(), true), true);
         ...
         String[] judul=new String[]{"BILL TO","DESCRIPTION","CUR","AMOUNT","PAID AMOUNT","END BALANCE"};
          ...
          String[] cond=new String[0];
            ...
            if(gen.gettable(request.getParameter("tpx")).equalsIgnoreCase("Download")){
                  ...
                  if(gen.gettable(request.getParameter("tx")).equalsIgnoreCase("1")){
                    ...
                    java.util.Vector datax=sgen.getDataQuery(conn,"AREXCEL1",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),y11,y11,gen.gettable(ses.getAttribute("TxtErcode"))});
                    ...
                    ses.setAttribute("NUMBERXLS",jdx);
                      ...
                      java.util.Vector maxmth=sgen.getDataQuery(conn,tpx1+"MAX",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),y11});      
                      java.util.Vector judulx=gen.getElement(',',"BILLTO CODE,BILLTO DESCRIPTION,CURR,"+y11+",");
							...
							<div class="col-xs-12">
								...
								<div>
									<div id="user-profile-3" class="user-profile row">
                						...
                						<form class="form-horizontal" name="BGA" method="POST" action="AR.jsp?tp=<%=tp%>" >
                                            ...
                                            <label class="control-label">&nbsp;Status AR:<select name="Y14" onchange="refresh();">
                                            <option></option>
										    ...
										    <label class="control-label">&nbsp;&nbsp;*Bill To:
                                                    ...
                                                    <input type="button" value="..." name="act1" style="font-weight: bold" onClick="setLink('BILLTO','Y12','SY6','<%=gen.gettable(ses.getAttribute("TxtErcode"))%>,','')">
                                             </label>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:down('2')">Download Unpaid AR</a>
											...
											<tbody>
												...
												<tr>
                                                       ...
                                                       %>
                                                            ...
                                                            <%if(ss==0){%>
                                                            <a href="ARDetail.jsp?tp=ARDETAIL&canedit=true&Y10=<%=y10%>&Y11=<%=y11%>&Y12=<%=Gen.gettable(request.getParameter("Y12"))%>&Y14=<%=Gen.gettable(request.getParameter("Y14"))%>&S1=<%=gen.gettable(data.get(s)).trim()%>&S2=<%=gen.gettable(data.get(s+1)).trim()%>&S3=<%=gen.gettable(data.get(s+5)).trim()%>&S4=<%=gen.gettable(data.get(s+2)).trim()%>"><%=gen.gettable(data.get(s+ss)).trim()%></a>
                                                            <%}else{
                                                                if(ss<=2){
