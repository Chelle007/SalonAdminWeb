...
<%
        ...
        java.io.PrintWriter log = new java.io.PrintWriter(new java.io.FileWriter(gen.getDataLogFileQuery(), true), true);
         ...
         String msg="";
            ...
            if(Gen.gettable(request.getParameter("act")).equalsIgnoreCase("Save")){
                ...
                for(int s=0;s<dold.size();s+=8){
                        if(request.getParameter("A"+hh)==null) break;
                      ...
                      if(gen.gettable(request.getParameter("A"+hh)).equalsIgnoreCase("ON")){
                        if(msg.length()==0) msg=sgen.update(conn,"APPAYMENTADD",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),S5,S6,Gen.gettable(request.getParameter("P")),S8,S7,Gen.gettable(request.getParameter("P1")),Gen.gettable(request.getParameter("P2"))});
                        ...
                        String[] xc=new String[]{S9,S8,"0","1","COST",Gen.gettable(request.getParameter("P1")),Gen.gettable(request.getParameter("P2")),S6,Gen.gettable(request.getParameter("S2")),Gen.gettable(request.getParameter("P1")),gen.gettable(ses.getAttribute("TxtErcode")),S7,Gen.gettable(request.getParameter("P")),SEQ,gen.gettable(request.getParameter("S4")),"0",Gen.gettable(request.getParameter("S1"))};
                        String rate="1";
                        ...
                        if(msg.length()==0) msg=sgen.update(conn,"TRX_BANKCOSTUPDATE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),S7,Gen.gettable(request.getParameter("P")),SEQ});
                        //[ErCode] ,[BillTo] ,curr,[seq],[jobno],[ErCode] ,[BillTo] ,curr,[seq],[jobno]
                        if(msg.length()==0) msg=sgen.update(conn,"APPAYUPDATE",new String[]{gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),S5,S6,gen.gettable(ses.getAttribute("TxtErcode")),Gen.gettable(request.getParameter("S1")),Gen.gettable(request.getParameter("S4")),S5,S6});
                        if(msg.length()==0) msg=sgen.update(conn,"LOGMENUPARMADD",new String[]{gen.gettable(ses.getAttribute("User")),"APPAYMENTADDH",gen.gettable(ses.getAttribute("TxtErcode"))+"|"+gen.gettable(request.getParameter("S1"))+"|"+Gen.gettable(request.getParameter("S4"))+"|"+S5+"|"+S6+"|"+Gen.gettable(request.getParameter("P"))+"|"});
         ...
         String[] judul=new String[]{"SEQ","JOBNO","ACCOUNT","RATE","AMOUNT","INVOICE NO","PAID AMT",""};
         
          String title="Account Payable";       
          String[] cond=new String[0];
          
          java.util.Vector balance=sgen.getDataQuery(conn,"APBALANCE"+gen.gettable(request.getParameter("Y14")),new String[]{gen.gettable(ses.getAttribute("TxtErcode")),gen.gettable(request.getParameter("S1")),gen.gettable(request.getParameter("S4"))});
          String bb="0";
          if(balance.size()>0){
    ...
    //    System.out.println(TPX+",="+gen.gettable(request.getParameter("Y14")));
...
%>
    ...
    <jsp:include page="menu.jsp" flush ="true"/>
			<div class="main-content">
				...
				<form class="form-horizontal" name="BGA" method="POST" action="APDetail.jsp?tp=<%=tp%>" >
								...
								<div>
									<div id="user-profile-3" class="user-profile row">
                                        <div class="col-sm-offset-1 col-sm-10">
                                            ...
                                            &nbsp;&nbsp;Pay status:<select  name="Y15" onchange="pp();">
                                            ...
                                            &nbsp;Paid Year:<input type="text" name="P3" size="4" maxlength="4" value="<%=dfp%>" onchange="pp();">
                                            </label>
                                            </div>
                                            <div>
                                            ...
                                            &nbsp;Pay No.:<input type="text" name="P1" size="15" maxlength="20" value="<%=Gen.gettable(request.getParameter("P1"))%>" ></label>
                                            &nbsp;Ref. No.:<input type="text" name="P2" size="15" maxlength="20" value="<%=Gen.gettable(request.getParameter("P2"))%>" ></label>
												...
												<tr>
                                                       ...
                                                       %>
                                                            <td <%if(ss==3 ||ss==4 ||ss==6) out.print("align=right");%>>
                                                                <%if(ss!=3 && ss!=4 && ss!=6){
                                                                    if(ss==1 && canedit){%>
                                                                    <a href="APPay.jsp?tp=APPAY&canedit=true&Y10=<%=y10%>&Y11=<%=y11%>&Y14=<%=Gen.gettable(request.getParameter("Y14"))%>&S1=<%=Gen.gettable(request.getParameter("S1"))%>&S2=<%=Gen.gettable(request.getParameter("S2"))%>&S3=<%=Gen.gettable(request.getParameter("S3"))%>&S4=<%=Gen.gettable(request.getParameter("S4"))%>&S5=<%=Gen.gettable(data.get(s))%>&S6=<%=Gen.gettable(data.get(s+1))%>&S7=<%=Gen.gettable(data.get(s+4))%>&S9=<%=Gen.gettable(data.get(s+2))%>"><%=gen.gettable(data.get(s+ss)).trim()%></a>
                                                                    <%}else{
